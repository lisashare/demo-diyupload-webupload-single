...
(function( $ ) {
    ...
    function createBox( $fileInput, file, webUploader ) {
        // var idcreate = $fileInput.attr('id');
        // if( $( '#fileBox_' + idcreate ) ){
        //     alert('上传文件数量超过限制!');
        // }
        // var test = 'fileBox_' + id ;
        // if(idcreate){
        ...
        //         </div>`;
        var idcreate = $fileInput.attr('id');
        var li = `<div id="fileBox_${idcreate}" class="diyUploadHover imgbase64"> 
                    <div class="viewThumb">
                        <input type="hidden">
                        <i class="diyCancel">
                        </i>
        ...
        var $fileBox = $parentFileBox.find('#fileBox_'+ idcreate);

        // 绑定取消事件；点击按钮删除图片
        var $diyCancel = $fileBox.find('.diyCancel').on('click',function(){
            console.log('delete');
            removeLi( $(this).parents('.imgbase64'), idcreate, webUploader );
        });
